...
const unsigned int libmodbus_version_micro = 10;
...
int _modbus_receive_msg(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type)
{
    ...
    if (ctx->debug) {
        if (msg_type == MSG_INDICATION) {
            debug_printf("Waiting for an indication...\n");
        } else {
            debug_printf("Waiting for a confirmation...\n");
        }
    }

    if (!ctx->backend->is_connected(ctx)) {
        if (ctx->debug) {
            debug_fprintf(stderr, "ERROR The connection is not established.\n");
        }
        return -1;
    }
    ...
    while (length_to_read != 0) {
        ...
        if (ctx->debug) {
            int i;
            for (i = 0; i < rc; i++)
                debug_printf("<%.2X>", msg[msg_length + i]);
        }
